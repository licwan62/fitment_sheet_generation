...
def score_candidate(record: InputRecord, title: str, url: str, cfg: dict, brand_aliases: dict | None = None,
                    model_aliases: dict | None = None, source_make: str = "",
                    source_name: str = "motorcyclespecs", source_priority: int = 1) -> Candidate:
    ...
    normalized_variants = [record_core_normalized]
    normalized_variants.extend(
        " ".join(token for token in normalize_name(value).split() if token not in ignored_words)
        for value in alias_values
    )
    normalized_variants = [value for value in normalized_variants if value]
    matched_variant = max(normalized_variants, key=lambda value: ratio(compact_name(value), candidate_model_compact))
    matched_compact = compact_name(matched_variant)
    matched_words = word_tokens(matched_variant)
    model_exact = matched_compact == candidate_model_compact
    ...
    primary_alpha_ok = not matched_words or bool(candidate_word_list and matched_words[0] == candidate_word_list[0])
    alpha_tokens_ok = matched_words == candidate_word_list
    ...
    keyword_ratio = ratio(" ".join(matched_words), " ".join(sorted(candidate_words))) / 100
    similarity = round(ratio(matched_compact, candidate_model_compact))
    ...
    reason = f"brand={'yes' if brand_ok else 'no'}; model_exact={'yes' if model_exact else 'no'}; model_similarity={similarity}; model_variant={matched_variant}; primary_alpha={'yes' if primary_alpha_ok else 'no'}; alpha_tokens={'yes' if alpha_tokens_ok else 'no'}; numbers={'yes' if number_ok else 'no'}; version={'yes' if version_ok else 'no'}; match_confidence={confidence}"
    return Candidate(
        record.input_id, title, url, source_make if brand_ok else "", title,
        score=score, status=status, reason=reason,
        source_name=source_name, source_priority=source_priority,
    )
...
def rank_candidates(record: InputRecord, pages: list[dict], cfg: dict, brand_aliases: dict | None = None, model_aliases: dict | None = None) -> list[Candidate]:
    ranked = [score_candidate(
        record, p["page_title"], p["page_url"], cfg, brand_aliases, model_aliases,
        p.get("brand_guess", ""), p.get("source_name", "motorcyclespecs"),
        int(p.get("source_priority", 1)),
    ) for p in pages]
    ranked.sort(key=lambda x: (-x.score, x.source_priority, x.url))
