...
try {
    ...
    $QClawScript = Join-Path $PSScriptRoot "..\qclaw_fitment_automation\qclaw_fitment_automation.ps1"
    if (-not (Test-Path -LiteralPath $QClawScript -PathType Leaf)) {
        throw "找不到 QClaw 自动化依赖：$QClawScript"
    }
    . $QClawScript `
