...
任务异常不再统一记录为“页面错误”。新运行会按异常原因写入以下状态；生成汇总时，旧日志中状态为“页面错误”的记录也会根据备注重新分类：

- `页面错误`：ChatGPT 页面明确显示 Something went wrong、网络错误等提示。保留 checkpoint，确认网络及页面恢复后重跑。
- `页面操作错误`：页面仍存在，但输入框、回复节点或 DOM 定位失败。保留当前对话 URL，重跑时从 checkpoint 恢复；若持续出现，需要更新页面定位规则。
- `浏览器错误`：Playwright context/browser 关闭、连接断开或无法新建标签页。脚本会重建浏览器会话；仍失败时停止整个项目，避免后续批次被连续标错。
- `回复超时`：超过 `runtime.max_reply_wait_seconds` 仍未取得稳定回复。检查原对话是否仍在生成，再从 checkpoint 续跑。
- `对话分支失败`：达到对话长度限制后找不到“在新聊天中分支”入口。脚本会重新加载父对话并最多重试 3 次；仍失败时保留原对话链，禁止静默创建丢失上下文的空白对话。
- `数据冲突`：同一 Ktype 出现不同外键等无法安全协调的问题。对于同名 `DIMENSION_GROUP_ID` 的三维冲突，程序会保留旧组，优先复用同系列下三维一致的已有组，否则用下一个稳定序号创建新组，并同步改写本批相关 Ktype 映射。
- `数据校验失败`：最终 TSV 表头、列数、ID 唯一性或字段格式不合格。先修复输出结构再重跑。
- `结果不完整`：回复缺少两张完整 TSV 或指定下载链接。可从 checkpoint 继续要求补齐结果。
- `脚本错误`：未匹配到以上类型的程序异常。根据日志备注和结果文件中的“脚本异常”段落定位。

所有非成功任务都会保留结果 Markdown、checkpoint、对话 URL 和异常备注。重新运行同一配置时仅跳过成功任务，其余任务会按 checkpoint 继续。

checkpoint 恢复时优先读取结果 Markdown 中最后一个已落盘的 `Round`：

1. 最后回复已经包含 COMPLETE、两张完整 TSV 和下载链接时，直接重新执行本地校验、尺寸组协调和累计表写入；不依赖原网页 DOM。
2. 最后回复尚未完成时，以该回复作为续跑依据，打开 checkpoint 保存的原对话继续。
3. 页面暂时找不到复制按钮或助手消息节点时，回退到本地最后回复，不会因此创建新对话。
4. 原页面和本地结果都没有可恢复回复时，保留 checkpoint 并停止该任务，禁止自动新建对话重发，以免重复研究和产生分叉结果。
