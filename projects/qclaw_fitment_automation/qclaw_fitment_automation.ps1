...
function Get-TaskBranchHandoffMessage {
    ...
    $pendingProgress = [ordered]@{}
    foreach ($ktype in $pendingSet.Keys) {
        $property = $progress.ktype_progress.PSObject.Properties[[string]$ktype]
        if ($property) { $pendingProgress[[string]$ktype] = $property.Value }
    }
    $pendingSummary = [ordered]@{
        checkpoint_revision = [int]$progress.revision
        ready_ktype_count = [int]$progress.progress.ready_ktype_count
        pending_ktype_count = [int]$progress.progress.pending_ktype_count
        pending_ktypes = @($progress.progress.pending_ktypes)
        ktype_progress = $pendingProgress
    } | ConvertTo-Json -Depth 10
    return @"
【Checkpoint 续跑交接】
这是新对话中的独立续跑任务。不要恢复、读取或回忆旧聊天；以下本地 checkpoint 是唯一可信进度。
只处理 PENDING TSV 中的 Ktype；不得重新研究 READY 项或已闭合尺寸组。
